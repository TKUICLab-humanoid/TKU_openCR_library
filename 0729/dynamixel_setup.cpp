...
void control::motor_address(){
  ...
  for (int id : {DXL20_ID, DXL21_ID, DXL26_ID, DXL27_ID}) {
    ...
    packetHandler->write2ByteTxRx(portHandler, id, ADDR_PRO_X_INDIRECTADDRESS_FOR_WRITE + 8, ADDR_PRO_X_PROFILE_VELOCITY + 4);
    packetHandler->write2ByteTxRx(portHandler, id, ADDR_PRO_X_INDIRECTADDRESS_FOR_WRITE + 10, ADDR_PRO_X_PROFILE_VELOCITY + 5);
  }
  // for pro_series
...
void control::motor_torque(){
  ...
  for (int id = 15;id < 27;id++) {
    groupSyncWrite.addParam(id+1, param);
  }
  groupSyncWrite.txPacket();
  groupSyncWrite.clearParam();
}

void control::motor_torque_disable(){
  // Initialize GroupSyncWrite instance to initial data adress
  dynamixel::GroupSyncWrite groupSyncWrite(portHandler, packetHandler, ADDR_PRO_INDIRECTDATA_FOR_TORQUE, LEN_PRO_GOAL_AND_VELOCITY);
  uint8_t dxl_error = 0;
  uint8_t param[1] = {TORQUE_DISABLE};;
  // for X-series, the adress is velocity-position
...
void control::motor_groupSyncWrite(uint8_t position_velocity[96]){
  // Initialize GroupSyncWrite instance to initial data adress
...
void control::tranfer_bit(uint8_t position_velocity[96]){//計算相差值
  int now_position[12];
  for (int i = 0;i < 12;i++) {
    now_position[i] = 
    ...
    relative_position[i] = now_position[i]-origin_position[i];
  }
}
...
void control::motor_speed(int motion_delay, int RL){
  ...
  for(int i = 0;i < 4;i++){
    profile_velocity[RL+i] = 10300;
    profile_velocity[RL+1] = 10300;
    profile_velocity[RL+2] = 3000;
    profile_velocity[RL+3] = 0;
