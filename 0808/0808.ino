...
static void Thread_Control(void const *argument) {
  ...
  float osc_lockrange, base_default_z, now_stand_height, now_com_height,hip_roll,ankle_roll;
  bool stand_balance;
  uint8_t footer;
  ...
  while (1) {
    ...
    if (header == 0x48 && Serial.available() >= sizeof(float)*9 + sizeof(bool) + 1) {
        // 原 0x48 Loading
        ...
        Serial.readBytes((char*)&hip_roll,       sizeof(float));
        Serial.readBytes((char*)&ankle_roll,     sizeof(float));
        ...
        if (footer == 0x45) {
          ...
          walking.compensation_swing_hip = hip_roll;
          walking.compensation_swing_ankle = ankle_roll;
          Serial.println("#ack=loading&send");
        } else {
          Serial.println("#ack=error");
        }

      }else if (header == 0x46 && Serial.available() >= sizeof(float)*10 + sizeof(bool) + 1) {
        ///lcup
        Serial.readBytes((char*)&com_y_swing,    sizeof(float));
        Serial.readBytes((char*)&y_swing_range,  sizeof(float));
        Serial.readBytes((char*)&period_t_,       sizeof(float));
      ...
      }else if (header == 0x50 && Serial.available() >= sizeof(float)*10 + sizeof(bool) + 1) {
        /*
        0x50,
        p["com_y_swing"],
        p["y_swing_range"],
        ...
        */
        ...
        footer = Serial.read();
        if (footer == 0x45) {
          walking.step_length  = x / 1000;
          walking.shift_length = y / 1000;
