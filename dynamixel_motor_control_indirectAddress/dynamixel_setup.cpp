...
void control::motor_address(){
  ...
  for (int id : {DXL19_ID, DXL20_ID, DXL25_ID, DXL26_ID}) {
    // setup indrectadress
  ...
  // for pro_series
  for (int id : {DXL15_ID, DXL16_ID, DXL17_ID, DXL18_ID, DXL21_ID, DXL22_ID, DXL23_ID, DXL24_ID}) {
    // setup indrectadress
...
void control::motor_groupSyncWrite(int position_velocity[208]){
  // Initialize GroupSyncWrite instance to initial data adress
...
void control::rad2motor(int RL){
  ...
  if (RL == 0) {
    motor_position[0] = 5000-motor_position[0];
    motor_position[3] = -motor_position[3];
    motor_position[4] = 0+2048+motor_position[4];
    motor_position[5] = 0+2048+motor_position[5];
  }
  else {
    motor_position[RL+0] = -5000+motor_position[RL+0];
    motor_position[RL+3] = motor_position[RL+3];
    motor_position[RL+4] = -0+2048-motor_position[RL+4];
    motor_position[RL+5] = -0+2048-motor_position[RL+5];
  }
  motor_position[RL+2] = -motor_position[RL+2];
}
