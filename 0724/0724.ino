...
void setup() {
  ...
  osThreadDef(THREAD_IMU,     Thread_IMU,     osPriorityNormal, 0, 2048);
  osThreadDef(THREAD_COMMAND, Thread_Command, osPriorityNormal, 0, 2048);
  osThreadDef(THREAD_SWITCH, Thread_Switch,   osPriorityLow, 0, 512);

  thread_id_imu     = osThreadCreate(osThread(THREAD_IMU),     nullptr);
  thread_id_cmd     = osThreadCreate(osThread(THREAD_COMMAND), nullptr);
...
static void Thread_Control(void const *argument) {
  ...
  while (1) {
    ...
    if (header == 0x48 && Serial.available() >= sizeof(float) * 7 + sizeof(bool) + 1) {
        size_t n = 0;
        n += Serial.readBytes((char*)&com_y_swing,    sizeof(float));
        n += Serial.readBytes((char*)&y_swing_range,  sizeof(float));
        n += Serial.readBytes((char*)&period_t_,        sizeof(float));
        n += Serial.readBytes((char*)&osc_lockrange,  sizeof(float));
        n += Serial.readBytes((char*)&base_default_z, sizeof(float));
        n += Serial.readBytes((char*)&now_stand_height,sizeof(float));
        n += Serial.readBytes((char*)&now_com_height, sizeof(float));
        n += Serial.readBytes((char*)&stand_balance,  sizeof(bool));

        if (n == sizeof(float) * 7 + sizeof(bool)) {
            footer = Serial.read();
            if (footer == 0x45) {
                setupControl.motor_address();
                setupControl.motor_torque();
    ...
    if (state == 4) {
      ...
      setupControl.walking_groupSyncWrite(setupControl.profile_velocity,
                                          setupControl.motor_position);
      osDelay(10);
      state = 0;
