...
void control::motor_groupSyncWrite(uint8_t position_velocity[216]){
  // Initialize GroupSyncWrite instance to initial data adress
...
void control::tranfer_bit(uint8_t position_velocity[216]){//計算相差值
  for (int i = 15;i < 27;i++) {
    ...
    relative_position[i-15] = relative_position[i-15]-origin_position[i-15];
  }
}
